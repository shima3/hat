...
\end{itemize}
上記のように拡張したBNFで，Hatプログラム\NT{\tt program}の構文を以下に示す．
\begin{hang}\tt % \item 019IOlq
\item\NT{program}\lrto\par
  \NT{includer}$^*$ \NT{definition}$^*$
\item\NT{includer}\lrto
  (include \NT{string})
\item\NT{definition}\lrto\par
  (defun \NT{identifier} \NT{hat function})
\item\NT{hat function}\lrto\hat\ \NT{head} \NT{body}
\item\NT{head}\lrto(\NT{identifier}$^*$)
\item\NT{body}\lrto\NT{expression}$^+$ | \NT{expression}$^+$ \NT{hat function}
\item\NT{body}\lrto\NT{command} | \NT{command} \NT{hat function}
\item\NT{command}\lrto\NT{operator}\NT{operand}$^*$
\item\NT{expression}\lrto\NT{identifier} \| \NT{datum} \| (\NT{hat function})
\end{hang}
