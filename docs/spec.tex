...
\item\NT{head}\lrto
  (\NT{identifier}$^*$)
\item\NT{body}\lrto\NT{operator} \NT{operands}
\item\NT{operator}\lrto\NT{identifier}\|(\NT{hat expression})\|(\NT{operands})
\item\NT{operands}\lrto\NT{expression}$^*$
  \|\NT{expression}$^*$ \NT{hat expression}
\item\NT{expression}\lrto
  \NT{datum} \| (\NT{hat expression}) \| (\NT{operands})
\item\NT{datum}\lrto
  \NT{number} \| \NT{string} \| \NT{identifier} \par
  \| (\NT{datum}$^*$)
\end{hang}
