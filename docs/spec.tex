...
\end{itemize}
上記のように拡張したBNFで，Hatプログラム\NT{\tt hat program}の構文を以下に示す．
\begin{hang}\tt % \item 019IOlq
\item\NT{hat program}\lrto\par
  \NT{includer}$^*$ \NT{hat definition}$^*$
\item\NT{includer}\lrto
  (include \NT{string})
\item\NT{hat definition}\lrto\par
  (defineCPS \NT{identifier} \NT{hat function}$^+$)
\item\NT{hat function}\lrto
  \hat\ \NT{hat head} \NT{hat body}
\item\NT{hat head}\lrto(\NT{identifier}$^*$)
\item\NT{hat body}\lrto\NT{hat expression}$^+$
\item\NT{hat expression}\lrto\NT{identifier} \| \NT{datum} \| (\NT{hat function}$^+$)
\end{hang}
